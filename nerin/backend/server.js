...
function savePurchaseOrders(purchaseOrders) {
  const dataPath = path.join(__dirname, '../data/purchase_orders.json');
  fs.writeFileSync(dataPath, JSON.stringify({ purchaseOrders }, null, 2), 'utf8');
}
...
function calculateDetailedAnalytics() {
  ...
  orders.forEach((order) => {
    order.items.forEach((item) => {
      ...
      if (prod) {
        ...
        salesByCategory[cat] = (salesByCategory[cat] || 0) + item.quantity * (item.price || prod.price_minorista);
        // Producto
        salesByProduct[prod.name] = (salesByProduct[prod.name] || 0) + item.quantity;
      }
    });
    // Total por cliente
  ...
  returns.forEach((ret) => {
    ret.items.forEach((item) => {
      const prod = products.find((p) => p.id === item.id);
      if (prod) {
        returnsByProduct[prod.name] = (returnsByProduct[prod.name] || 0) + item.quantity;
      }
    });
  });
  // Top clientes
...
const server = http.createServer((req, res) => {
    ...
    req.on('end', () => {
      try {
        ...
        if (user) {
          ...
          return sendJson(res, 401, { success: false, message: 'Credenciales incorrectas' });
        }
      } catch (err) {
        console.error(err);
    ...
    req.on('end', () => {
      try {
        ...
        if (!Array.isArray(cart) || cart.length === 0) {
          return sendJson(res, 400, { error: 'El carrito está vacío o no es válido' });
        }
        ...
        try {
          ...
          cart.forEach((item) => {
            ...
            if (index !== -1) {
              ...
              // Restar de almacenes si existen
              if (products[index].warehouseStock) {
                ...
                if (whs.central && whs.central > 0) {
                  const toDeduct = Math.min(item.quantity, whs.central);
                ...
                for (const w in whs) {
                  ...
                  const avail = whs[w];
                  const deduct = Math.min(item.quantity, avail);
                  whs[w] -= deduct;
        ...
        return sendJson(res, 200, { success: true, message: 'Pedido registrado', orderId });
      } catch (err) {
        console.error(err);
  ...
  if (pathname === '/api/orders' && req.method === 'GET') {
    try {
      ...
      console.error(err);
      return sendJson(res, 500, { error: 'No se pudieron obtener los pedidos' });
    }
  }
  ...
  if (pathname === '/api/products' && req.method === 'POST') {
    ...
    req.on('end', () => {
      try {
        ...
        const newId = (products.length ? Math.max(...products.map((p) => parseInt(p.id, 10))) + 1 : 1).toString();
        newProduct.id = newId;
    ...
    try {
      ...
      orders.forEach((order) => {
        ...
        order.items.forEach((item) => {
          ...
          salesByMonth[monthKey] = (salesByMonth[monthKey] || 0) + qty * item.price;
      ...
      console.error(err);
      return sendJson(res, 500, { error: 'No se pudieron calcular las métricas' });
    }
  ...
  if (pathname === '/api/clients' && req.method === 'GET') {
    try {
      ...
      console.error(err);
      return sendJson(res, 500, { error: 'No se pudieron obtener los clientes' });
    }
  }
  ...
  if (pathname.startsWith('/api/invoices/') && req.method === 'POST') {
    ...
    try {
      ...
      if (order.customer && order.customer.email) {
        ...
        if (client) {
          clientInfo = { ...client };
          if (client.condicion_iva && client.condicion_iva.toLowerCase().includes('responsable')) {
            type = 'A';
          }
        }
      }
  ...
  if (pathname === '/api/returns' && req.method === 'POST') {
    ...
    req.on('end', () => {
      try {
        const { orderId, reason, items, customerEmail } = JSON.parse(body || '{}');
        if (!orderId || !reason) {
          return sendJson(res, 400, { error: 'Faltan parámetros para la devolución' });
        }
        ...
        if (customerEmail && order.customer && order.customer.email !== customerEmail) {
          return sendJson(res, 403, { error: 'No puedes devolver un pedido que no es tuyo' });
        }
        if (order.status !== 'entregado') {
          return sendJson(res, 400, { error: 'Sólo se pueden devolver pedidos entregados' });
        }
        ...
        if (clientBlocked) {
          return sendJson(res, 403, { error: 'Cliente bloqueado para devoluciones por actividades sospechosas' });
        }
        ...
        if (clientReturns.length >= 3) {
          ...
          return sendJson(res, 403, { error: 'Cliente bloqueado debido a exceso de devoluciones' });
        }
        // Crear ID de devolución
        const returnId =
          'RET-' + Date.now().toString(36) + '-' + Math.floor(Math.random() * 1000);
        const newReturn = {
          id: returnId,
          orderId,
          customerEmail: email || '',
  ...
  if (pathname === '/api/returns' && req.method === 'GET') {
    try {
      ...
      console.error(err);
      return sendJson(res, 500, { error: 'No se pudieron obtener las devoluciones' });
    }
  }
  ...
  if (pathname.startsWith('/api/returns/') && req.method === 'PUT') {
    ...
    req.on('end', () => {
      try {
        ...
        saveReturns(returns);
        return sendJson(res, 200, { success: true, returnRequest: returns[idx] });
      } catch (err) {
        console.error(err);
        return sendJson(res, 400, { error: 'Solicitud inválida' });
      }
  ...
  // API: registrar nuevo cliente mayorista
  // Ruta: /api/register (POST)
  if (pathname === '/api/register' && req.method === 'POST') {
    ...
    req.on('end', () => {
      try {
        ...
        if (!client.email || !client.name || !client.cuit) {
        ...
        if (clients.some((c) => c.email === client.email)) {
          return sendJson(res, 409, { error: 'El cliente ya existe' });
        }
        // Crear nuevo registro con saldo cero y estado pendiente de aprobación
        const newClient = {
          email: client.email,
          name: client.name,
          cuit: client.cuit,
  ...
  if (pathname === '/api/config' && req.method === 'GET') {
    try {
      ...
      console.error(err);
      return sendJson(res, 500, { error: 'No se pudo obtener la configuración' });
    }
  }
  ...
  if (pathname === '/api/suppliers' && req.method === 'GET') {
    try {
      ...
      console.error(err);
      return sendJson(res, 500, { error: 'No se pudieron obtener los proveedores' });
    }
  }
  ...
  if (pathname === '/api/suppliers' && req.method === 'POST') {
    ...
    req.on('end', () => {
      try {
        ...
        if (!newSup.name) {
          return sendJson(res, 400, { error: 'Se requiere el nombre del proveedor' });
        }
        const suppliers = getSuppliers();
        // Generar ID incremental
        const newId = suppliers.length
          ? (Math.max(...suppliers.map((s) => parseInt(s.id, 10))) + 1).toString()
          : '1';
        newSup.id = newId;
  ...
  if (pathname === '/api/purchase-orders' && req.method === 'GET') {
    try {
      ...
      console.error(err);
      return sendJson(res, 500, { error: 'No se pudieron obtener las órdenes de compra' });
    }
  }
  if (pathname === '/api/purchase-orders' && req.method === 'POST') {
    ...
    req.on('end', () => {
      try {
        ...
        if (!order.supplier || !Array.isArray(order.items) || order.items.length === 0) {
          return sendJson(res, 400, { error: 'Datos incompletos para la orden de compra' });
        }
        ...
        if (!wasReceived && willBeReceived) {
          ...
          try {
            ...
            existing.items.forEach((item) => {
              const pIdx = products.findIndex((p) => p.sku === item.sku || p.id === item.id);
              if (pIdx !== -1) {
                // Sumar stock global
                ...
                if (!products[pIdx].warehouseStock) {
                  ...
                  products[pIdx].warehouseStock.central =
                    (products[pIdx].warehouseStock.central || 0) + item.quantity;
                }
                modified = true;
              }
            });
            ...
            console.error('Error al actualizar inventario tras orden de compra:', invErr);
          }
        }
        return sendJson(res, 200, { success: true, purchaseOrder: orders[idx] });
      } catch (err) {
        console.error(err);
        return sendJson(res, 400, { error: 'Solicitud inválida' });
      }
  ...
  if (pathname === '/api/analytics/detailed' && req.method === 'GET') {
    try {
      ...
      console.error(err);
      return sendJson(res, 500, { error: 'No se pudieron calcular las analíticas detalladas' });
    }
  }
